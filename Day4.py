...
for i in parsed:
    ...
    if fields == 7:
        ...
        byr = int(i[i.index("byr:") + 4 : i.find(" ",i.index("byr:"),-1)])
        iyr = int(i[i.index("iyr:") + 4 : i.find(" ",i.index("iyr:"),-1)])
        eyr = int(i[i.index("eyr:") + 4 : i.find(" ",i.index("eyr:"),-1)])
        hgt = i[i.index("hgt:") + 4 : i.find(" ",i.index("hgt:"),-1)]
        hcl = i[i.index("hcl:") + 4 : i.find(" ",i.index("hcl:"),-1)]
        ecl = i[i.index("ecl:") + 4 : i.find(" ",i.index("ecl:"),-1)]
        pid = i[i.index("pid:") + 4 : i.find(" ",i.index("pid:"),-1)]
        valid = 0
        ...
        if hgt.count("in") == 1:
            ...
            height_in = int(hgt[0 : hgt.index("i")])

            if height_in >= 59 and height_in <= 76:
                valid = valid + 1
            
        if hgt.count("cm") == 1:

            height_cm =  int(hgt[0 : hgt.index("c")])

            if height_cm >= 150 and height_cm <= 193:
                valid = valid + 1

        for j in characters:
            count = hcl.count(j)
            valid_characters =  valid_characters + count
        
        if hcl.count("#") == 1 and len(hcl) == 7 and valid_characters == 6:
            valid = valid + 1

        for k in colors:
            valid_colors = ecl.count(k)
            if valid_colors == 1:
                valid = valid + 1
